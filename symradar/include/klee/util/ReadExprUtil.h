...
private:
  std::map<const ReadExpr *, ref<Expr>> &replacements;

public:
  ReadExprReplacer(std::map<const ReadExpr *, ref<Expr>> &replacements)
      : replacements(replacements) {}
  ref<Expr> replcaeReadExpr(const ref<Expr> &expr) { return visit(expr); }
  Action visitRead(const ReadExpr &re) override {
    auto it = replacements.find(&re);
    if (it != replacements.end()) {
      // Replace the ReadExpr with its corresponding replacement
