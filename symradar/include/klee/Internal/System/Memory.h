...
void pointerToData(std::vector<unsigned char> &data, uint64_t ptr);
uint64_t dataToPointer(const std::vector<unsigned char> &data);
...
class LazyInitManager {
  ...
  std::map<const Array *, MemArchive2> originalObjects;
  ...
  std::map<std::pair<const Array *, uint32_t>,
           std::tuple<uint64_t, const MemoryObject *, const Array *, bool>>
      symbolicPointerMap2; // bool: existing - if not, use symbolic size
  std::map<const Array *, std::vector<uint8_t>>
      symbolicDataMap; // Symbolized object
  std::map<ref<Expr>, const MemoryObject *>
      base_addrs; // Symbolic address to MemoryObject mapping

public:
  LazyInitManager() {}

  void trackBaseAddr(ref<Expr> address, const MemoryObject *mo) {
    if (base_addrs.find(address) == base_addrs.end()) {
      base_addrs[address] = mo;
    }
  }

  const MemoryObject *getObjectFromSymbolicAddr(ref<Expr> address) {
    if (base_addrs.find(address) != base_addrs.end()) {
      return base_addrs[address];
    }
    return nullptr;
  }

  void addSymbolicPointer(const Array *array, uint32_t readOffset,
                          uint64_t originalAddr, const MemoryObject *mo,
                          const Array *moArray, bool existing,
                          ref<Expr> addrExpr) {
    symbolicPointerMap[array] =
        std::make_tuple(originalAddr, mo, moArray, existing);
    if (symbolicPointerMap2.find(std::make_pair(array, readOffset)) ==
        symbolicPointerMap2.end()) {
      SPDLOG_INFO("[LazyInitManager] [sym-ptr] [arr {}] [offset {}] [addr {}] "
                  "[mo {}] [existing {}]",
                  array->getName(), readOffset, originalAddr, mo->name,
                  existing);
    }
    trackBaseAddr(addrExpr, mo);
    symbolicPointerMap2[std::make_pair(array, readOffset)] =
        std::make_tuple(originalAddr, mo, moArray, existing);
  }

  std::tuple<uint64_t, const MemoryObject *, const Array *, bool>
  getSymbolicPointer(const Array *array, uint32_t readOffset = 0) {
    if (symbolicPointerMap2.find(std::make_pair(array, readOffset)) !=
        symbolicPointerMap2.end()) {
      return symbolicPointerMap2[std::make_pair(array, readOffset)];
    }
    return std::make_tuple<uint64_t, const MemoryObject *, const Array *, bool>(
        0, nullptr, nullptr, false);
  }

  std::map<const Array *, MemArchive2> &getOriginalObjectMap() {
  ...
  MemoryObject *getLazyInitAssignment(ref<Expr> addrExpr,
                                      uint64_t requestedAddress) {

    // First, look for the address in the base_addrs map
    if (base_addrs.find(addrExpr) != base_addrs.end()) {
      const MemoryObject *mo = base_addrs[addrExpr];
      return (MemoryObject *)mo; // Cast to MemoryObject
    }
    if (lazyInitAssignments.find(requestedAddress) ==
        lazyInitAssignments.end()) {
      return nullptr;
    }
    return lazyInitAssignments[requestedAddress];
  }
  bool isLazyInitObject(MemoryObject *mo) {
    return lazyInitObjectsSet.find(mo) != lazyInitObjectsSet.end();
  }
  void addOriginalObject(const Array *array, MemArchive2 memArc) {
    originalObjects[array] = memArc;
  }
  bool getOriginalObject(const Array *array, MemArchive2 &memArc) {
