...
#define addDNR(name, handler)                                                  \
  { name, &SpecialFunctionHandler::handler, true, false, false }
    ...
    add("uni_klee_restore_concrete", handleRestoreConcrete, false),
    add("__uni_klee_poc_choice", handleUniKleePocChoice, true),
    add("__uni_klee_poc_record_value", handleUniKleePocRecordValue, false),
...
void SpecialFunctionHandler::handleRestoreConcrete(
    ExecutionState &state, KInstruction *target,
    std::vector<ref<Expr>> &arguments) {
  if (arguments.size() != 2) {
    executor.terminateStateOnError(state,
                                   "Incorrect number of arguments to "
                                   "uni_klee_restore_concrete(void*, size_t)",
                                   Executor::User);
    return;
  } else {
    ref<Expr> address = arguments[0];
    ...
    SPDLOG_DEBUG("[state {}] uni_klee_restore_concrete {} size {}(addr {})",
                 state.getID(), address->str(), arguments[1]->str(),
                 op.first->address);
    ConstantExpr *CE = dyn_cast<ConstantExpr>(size);
    if (!isa<ConstantExpr>(address)) {
      return;
    ...
    uint64_t offset =
        cast<ConstantExpr>(address)->getZExtValue() - op.first->address;
    const Array *array = nullptr;
    for (auto &it : state.symbolics) {
      if (it.getMemoryObject() == op.first) {
        array = it.getArray();
        break;
      }
    }
    ref<Expr> val = op.second->read(offset, CE->getZExtValue() * 8);
    if (isa<ConstantExpr>(val)) {
      // If the value is constant, skip
      SPDLOG_DEBUG("[state {}] uni_klee_restore_concrete: value is constant",
                   state.getID());
      return;
    }
    if (array == nullptr) {
      executor.terminateStateOnError(
          state, "restore_concrete: no symbolic object found", Executor::User);
      return;
    }
    MemArchive2 ma;
    if (!executor.lazyInitManager.getOriginalObject(array, ma)) {
      executor.terminateStateOnError(
          state, "restore_concrete: no original object found", Executor::User);
      return;
    }
...
void SpecialFunctionHandler::handleUniKleePocChoice(
    ExecutionState &state, KInstruction *target,
    std::vector<ref<Expr>> &arguments) {
  if (arguments.size() != 0) {
    executor.terminateStateOnError(state,
                                   "Incorrect number of arguments to "
                                   "__uni_klee_poc_choice()",
                                   Executor::User);
    return;
  } else {
    executor.executeUniKleePocChoice(state, target);
  }
}

// Record a value in the poc to check regression errors
// Type 0: placeholder
// Type 1: integer
// Type 2: pointer
void SpecialFunctionHandler::handleUniKleePocRecordValue(
    ExecutionState &state, KInstruction *target,
    std::vector<ref<Expr>> &arguments) {
  if (arguments.size() != 5) {
    executor.terminateStateOnError(
        state,
        "Incorrect number of arguments to "
        "uni_klee_poc_record_value(unsigned long long value, int size, int "
        "type, char *name, int loc)",
        Executor::User);
    return;
  }
  ref<Expr> value = arguments[0];
  uint64_t size = dyn_cast<ConstantExpr>(arguments[1])->getZExtValue();
  uint64_t type = dyn_cast<ConstantExpr>(arguments[2])->getZExtValue();
  ...
  if (!isa<ConstantExpr>(value)) {
    ...
    if (!isa<ConstantExpr>(value)) {
      if (type == 2) {
        // Symbolic pointer: let's assume it's null
        bool res;
        bool success = executor.solver->mayBeTrue(
            state,
            EqExpr::create(value, ConstantExpr::create(0, value->getWidth())),
            res);
        if (!success) {
          executor.terminateStateOnError(
              state, "klee_poc_record_value: mayBeTrue failed", Executor::User);
          return;
        }
        if (res) {
          value = ConstantExpr::create(0, value->getWidth());
        }
      }
    }
  }
  // Symbolic values... How?
  // 1. Store the symbolic value in a map in targetStack
  // 2. Compare if original and patched value are not crash
  if (isa<ConstantExpr>(value)) {
    klee_log_data(fmt::format("[poc] [rec] [state {}] [name {}] [sym 0] [val "
                              "{}] [size {}] [type {}] [loc {}]",
                              state.getID(), name, value->str(), size, type,
                              loc));
  } else {
    value =
        executor.toConstant(state, value, "uni_klee_poc_record_value", false);
    klee_log_data(fmt::format("[poc] [rec] [state {}] [name {}] [sym 1] [val "
                              "{}] [size {}] [type {}] [loc {}]",
                              state.getID(), name, value->str(), size, type,
                              loc));
  }
