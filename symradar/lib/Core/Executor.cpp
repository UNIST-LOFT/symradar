...
cl::opt<bool> NonCondPatch("non-cond-patch", cl::init(false),
                           cl::desc("Use non conditional patch"));
...
void Executor::StateGroup::continueAll() {
  ...
  if (!resume) {
    SPDLOG_DEBUG("[pig] continueAll: no states to resume");
    if (st == StateType::ST_base_after) {
      st = StateType::ST_base;
      for (ExecutionState *s : executor->states) {
        if (s->metaData.isType(st) && s->metaData.getIsPaused() &&
            s->ptreeNode->validChildren == 0) {
          executor->continueState(*s);
  ...
  if (!resume) {
    SPDLOG_WARN("[pig] continueAll: no states to resume, all paused");
...
Executor::setModule(std::vector<std::unique_ptr<llvm::Module>> &modules,
  ...
  if (TargetFunction.empty() != true) {
    ...
    if (NonCondPatch) {
      patchLocationAddr = finalModule->getFunction("__uni_klee_poc_choice");
    } else {
...
// Select patch for NonCondPatch
void Executor::executeUniKleePocChoice(ExecutionState &state,
                                       KInstruction *target) {
  // First time we reach this instruction, select a patch
  if (state.metaData.getPatchID() < 0) {
    // Almost same as selectPatchID
    ...
    SPDLOG_DEBUG("[state {}] select patch", state.getID());
    ...
    for (int i = 0; i < patchSize; i++) {
      ...
      if (i == patchSize - 1) {
        ...
        bindLocal(target, *targetState, pid);
        break;
      }
      ExecutionState *s = cloneState(targetState);
      if (i == 0) { // Use first patch
        ExecutionState *es = targetState;
        es->metaData.setPatchID(patch);
        bindLocal(target, *es, pid);
        ExecutionState *ns = cloneState(es);
        bindLocal(target, *ns, pid);
    ...
    // We have already selected a patch, just bind it
    ref<ConstantExpr> value =
        ConstantExpr::alloc(state.metaData.getPatchID(), Expr::Int32);
...
void Executor::initUniKleeAtEntry(ExecutionState &state, const KFunction *kf) {
  ...
  llvm::Function *rf =
      kmodule->module->getFunction("uni_klee_restore_concrete");
  ...
  KInstruction *kinst;
  if (NonCondPatch) {
    kinst = cfg->findTargetInst("__uni_klee_poc_choice");
  } else {
    kinst = cfg->findTargetInst("__cpr_choice");
  }
...
void Executor::executeCall(ExecutionState &state, KInstruction *ki, Function *f,
  ...
  if (f && f->isDeclaration()) {
    ...
    case Intrinsic::fshl: {
      ...
      assert(w == op2->getWidth() &&
             "fshr/fshl operands must have the same width");
      assert(w == op3->getWidth() &&
             "fshr/fshl shift amount must have the same width");
    ...
    if (kmodule->functionMap.count(f) == 0) {
      terminateStateOnExecError(state,
                                "calling function with no KFunction entry");
      return;
    }
    ...
    if (f == patchLocationAddr && !NonCondPatch) {
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
    ...
    if (state.stack.size() <= 1) {
      ...
      if (!isVoidReturn) {
        ...
        if (t != Type::getVoidTy(i->getContext())) {
          ...
          if (ri->getFunction() == patchLocationAddr && !NonCondPatch) {
  ...
  case Instruction::Call: {
    ...
    if (f) { // Call function
      ...
      v = state.constraints.simplifyExpr(v);
      if (!isa<ConstantExpr>(v)) {
        // Try to find a concrete value
        ref<Expr> cv = getConcreteValue(state, v);
        v = state.constraints.simplifyExpr(cv);
      }
      if (ConstantExpr *CE = dyn_cast<ConstantExpr>(v)) {
        ...
        if (!hasInvalid) {
          if (legalFunctions.count((uint64_t)f) > 0) {
  ...
  case Instruction::GetElementPtr: {
    ...
    if (isa<ConstantExpr>(originalBase) && !isa<ConstantExpr>(base)) {
      // Concrete base + symbolic offset
      ObjectPair op;
      if (state.addressSpace.resolveOne(dyn_cast<ConstantExpr>(originalBase),
                                        op)) {
        lazyInitManager.trackBaseAddr(base, op.first);
      } else {
        SPDLOG_WARN("[state {}] GEP with invalid base address {} at {}",
                    state.getID(), originalBase->str(), sourceLoc);
      }
    } else if (!isa<ConstantExpr>(originalBase)) {
      if (const MemoryObject *mo =
              lazyInitManager.getObjectFromSymbolicAddr(originalBase)) {
        lazyInitManager.trackBaseAddr(base, mo);
      }
    }
    if (taint) {
      ...
      if (NonCondPatch) {
        ...
        if (!isVoidReturn) {
          ...
          if (isa<ConstantExpr>(result)) {
            klee_log_data(fmt::format(
                "[poc] [ret] [state {}] [value {}] [symbolic 0]", state.getID(),
                dyn_cast<ConstantExpr>(result)->getZExtValue()));
          } else {
            ref<ConstantExpr> ce =
                toConstant(state, result, "return value", false);
            klee_log_data(
                fmt::format("[poc] [ret] [state {}] [value {}] [symbolic 1]",
                            state.getID(), ce->getZExtValue()));
          }
        } else {
          klee_log_data(fmt::format(
              "[poc] [ret] [state {}] [value 0] [symbolic 0]", state.getID()));
        }
      }
    }
...
MemoryObject *Executor::lazyAllocation(ExecutionState &state, ref<Expr> addr,
                                       ...
                                       int depth, uint64_t requestedAddr) {
  SPDLOG_DEBUG("[state {}] lazyAllocation on {} - type {}", state.getID(),
               addr->str(), get_type_string(type));
  if (type->isPointerTy()) {
    ...
    uint64_t maxSize =
        (8 > typeSize ? 8 : typeSize) * 4; // 4 times the type size
    MemoryObject *mo =
        lazyInitManager.getLazyInitAssignment(addr, requestedAddr);
    const Array *newArray = nullptr;
    if (mo == nullptr) {
      // allocate memory
      mo = memory->allocate(maxSize, /*isLocal=*/false,
      ...
      for (auto &it : lazyInitManager.getOriginalObjectMap()) {
        if (it.second.mo == mo) {
          newArray = it.first;
          break;
        }
      ...
      assert(newArray != nullptr &&
             "Lazy allocation must have a symbolic array");
    ...
    if (result) {
      ...
      if (state.metaData.isTypeBase()) {
        ...
        state.shadowMemory.push_back(std::make_pair(cmo, newArray));
        SPDLOG_INFO("[state {}] Save to shadow address space: {}({}) with {}",
                    state.getID(), mo->name, mo->getBaseExpr()->getZExtValue(),
...
Executor::expandLazyAllocation(ExecutionState &state, ref<Expr> addr,
  ...
  // Never seen symbolic base -> allocate new object
  time::Span timeout = coreSolverTimeout;
  solver->setTimeout(timeout);
  if (base_type->isFirstClassType()) {
    ...
    if (depth >= MaxLazyDepth) {
      ...
      if (next_fork) {
        MemoryObject *mo = lazyAllocation(*next_fork, addr, type, target,
                                          typeSize, depth, requestedAddr);
        ref<Expr> result = ConstantExpr::createPointer(0);
        if (mo != nullptr) {
          result = mo->getBaseExpr();
        }
        bindLocal(target, *next_fork, result);
        return result;
      }
    }
  }
...
void Executor::terminateState(ExecutionState &state, std::string msg,
  ...
  if (check) {
    ...
    if (state.metaData.isType(StateType::ST_base_after)) {
      ...
      if (executionType != ExecutionType::MakeSnapshot) {
        if (state.metaData.getIsCrash() ||
            (!LazyPatch &&
             (suffixRef.endswith("ret") || suffixRef.endswith("exit")))) {
...
void Executor::callExternalFunction(ExecutionState &state, KInstruction *target,
  ...
  for (int i = 0, e = arguments.size(); i < e; ++i) {
    ...
    if (ExternalCalls ==
      ...
      llvm::Type *type = fType->getParamType(i);
      ref<Expr> ae = getConcreteValue(state, arg);
      ae = optimizer.optimizeExpr(ae, true);
      if (!isa<ConstantExpr>(ae)) {
        hasSymbolicArgs = true;
        break;
      }
      ref<ConstantExpr> ce;
      bool success = solver->getValue(state, ae, ce);
      if (!isa<ConstantExpr>(arg)) {
  ...
  if (hasSymbolicArgs) {
    // Pretend it was done: return symbolic value
    SPDLOG_DEBUG("[state {}] External call with symbolic args: {}",
                 state.getID(), function->getName().str());
    Type *resultType = target->inst->getType();
    if (resultType != Type::getVoidTy(function->getContext())) {
      ref<Expr> e = getSymbolicValue(
                        state, getWidthForLLVMType(resultType) / 8,
...
bool Executor::analyzeSymbolicAddr(ExecutionState &state, ref<Expr> address,
  ...
  if (const MemoryObject *mo =
          lazyInitManager.getObjectFromSymbolicAddr(address)) {
    // If the address is cached, use it
    uint64_t baseAddr = mo->address;
    baseAddresses.insert(baseAddr);
    SPDLOG_DEBUG("[state {}] Found baseAddr(cached): {} at {}", state.getID(),
                 baseAddr, address->str());
    return true;
  }
  ...
  for (const Array *arr : visitor.getArrays()) {
    auto itIdx = visitor.getArrayIndices().find(arr);
    // First, try to find the pointer read
    // If read from symbolic object, offset can be devided by 8
    if (itIdx != visitor.getArrayIndices().end()) {
      std::set<int> candidateOffsets;
      std::set<int> offsets;
      for (int idx : itIdx->second) {
        if (idx % 8 == 0) {
          candidateOffsets.insert(idx);
        }
        offsets.insert(idx);
      }
      // Check if we have read all 8 bytes
      for (int idx : candidateOffsets) {
        bool allRead = true;
        for (int i = 0; i < 8; i++) {
          if (offsets.count(idx + i) == 0) {
            allRead = false;
            break;
          }
        }
        if (allRead) {
          // We have read all 8 bytes, so it might be a pointer
          auto res = lazyInitManager.getSymbolicPointer(arr, idx);
          if (std::get<0>(res) != 0) {
            baseAddresses.insert(std::get<0>(res));
            found = true;
            SPDLOG_DEBUG("[state {}] Found baseAddr(symPtr): {} at offset {}",
                         state.getID(), std::get<0>(res), idx);
          }
        }
      }
    }
...
void Executor::handleSymbolicPointer(ExecutionState &state, bool isWrite,
  ...
  if (analyzeSymbolicAddr(*nonNullState, address, saVisitor, baseAddresses)) {
    ...
    MemoryObject *mo = lazyAllocation(*nonNullState, address,
                                      target->inst->getType(), target, 0, 0, 0);
  ...
  if (tryConcretize) {
    ...
    if (result) {
      ...
      if (inBounds) {
        lazyInitManager.trackBaseAddr(address, mo);
...
void Executor::executeMemoryOperation(
  ...
  if (!isa<ConstantExpr>(address)) {
    ...
    if (usingSeeds) {
      ...
      handleSymbolicPointer(state, isWrite, originalAddress, value, target);
      return;
    }
  }
...
std::string Executor::getConsistentName(ExecutionState &state,
                                        std::string prefix, uint64_t address,
                                        uint64_t size) {
  std::string name = fmt::format("{}_a{}_s{}", prefix, address, size);
  SPDLOG_DEBUG("[state {}] getConsistentName: {}", state.getID(), name);
  return name;
}

std::string Executor::getUniqueName(ExecutionState &state, std::string name,
                                    std::string prefix, uint64_t address) {
  std::string namePrefix = fmt::format("{}_{}", prefix, name);
  if (address)
...
void Executor::executeMakeSymbolic(ExecutionState &state,
                                   ...
                                   const std::string name, bool existing) {
  ...
  if (!replayKTest) {
    ...
    // }
    SPDLOG_INFO("[state {}] Make symbolic name: {}(object {}) at {} with "
                "size {} at {}",
                state.getID(), array->getName(), name, mo->address, mo->size,
...
const Array *Executor::executeMakeFieldSymbolic(ExecutionState &state,
                                                const MemoryObject *mo,
...
ref<Expr> Executor::getConcreteValue(ExecutionState &state, ref<Expr> e) {
  ...
  for (const ReadExpr *re : saVisitor.getReads()) {
    ...
    MemArchive2 ma;
    if (!lazyInitManager.getOriginalObject(array, ma)) {
      SPDLOG_WARN(
          "[state {}] getConcreteValue: no original object for array {}",
          state.getID(), array->getName());
    }
    ...
    if (isa<ConstantExpr>(index)) {
      ...
      if (idx < ma.getConcreteSize()) {
        ...
        SPDLOG_DEBUG("[state {}] getConcreteValue: index {} no original data "
                     "for array {}",
                     state.getID(), idx, array->getName());
      }
    }
  }
  // Replace the read expressions with concrete values
  ReadExprReplacer replacer(concreteValues);
  ref<Expr> concreteExpr = replacer.visit(e);
  solver->setTimeout(coreSolverTimeout);
  bool result;
  bool success =
      solver->mayBeTrue(state, EqExpr::create(e, concreteExpr), result);
  solver->setTimeout(time::Span());
  if (!success) {
    state.pc = state.prevPC;
    terminateStateEarly(state, "Query timed out (concrete value).");
    return e;
  }
  if (!result) {
    // Last try
...
int Executor::makeSymbolicFromSnapshot(ExecutionState &state,
  ...
  for (llvm::Function::const_arg_iterator ai = f->arg_begin(),
    ...
    if (TargetFunction == "main" ||
      ...
      std::vector<uint8_t> data;
      ref<ConstantExpr> ca = snapshot->arguments[index];
    ...
    for (int index = 0; index < SymbolizeBound; index++) {
      ...
      for (auto cur : currents) {
        ...
        for (PtrEdge *ptr :
          ...
          uint64_t destAddr = dpn->getAddress();
          if (snapshot->fn_ptrs_map.count(destAddr) > 0) {
            // Skip function pointers
            SPDLOG_DEBUG("[state {}] Skip function pointer: {} -> {}",
                         state.getID(), addr, destAddr);
            continue;
          }
  ...
  while (!terminalQueue.empty()) {
    ...
    for (uint64_t term : *level) {
      ...
      for (PtrEdge *pe :
        ...
        uint64_t dest = pe->getTo()->getAddress();
        if (snapshot->fn_ptrs_map.count(dest) > 0) {
          // Skip function pointers
          SPDLOG_DEBUG("[state {}] Skip function pointer: {}", state.getID(),
                       dest);
          continue;
        }
  ...
  if (SymbolizeLevel >= HighSymbolization ||
    ...
    for (auto &mr : snapshot->readAccess) {
      ...
      //   continue;
      // }
      const Array *arr = executeMakeFieldSymbolic(
          state, op.first, pointer - mr.base, mr.size, op.first->name, false);
      if (arr != nullptr) {
        symbolize.push_back(
