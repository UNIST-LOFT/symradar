...
void pointerToData(std::vector<unsigned char> &data, uint64_t ptr) {
  ...
  for (unsigned index = 0; index < ptr_width; ++index, ++addrBytes) {
    data.push_back(*addrBytes);
  }
}

uint64_t dataToPointer(const std::vector<unsigned char> &data) {
  assert(data.size() == Context::get().getPointerWidth() / 8);
  uintptr_t addr = 0;
  unsigned char *addrBytes = ((unsigned char *)&(addr));
  for (unsigned index = 0; index < data.size(); ++index, ++addrBytes) {
    *addrBytes = data[index];
  }
