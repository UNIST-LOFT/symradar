...
StackFrame::StackFrame(KInstIterator _caller, KFunction *_kf)
    : caller(_caller), kf(_kf), callPathNode(0), snapshot(nullptr),
...
StackFrame::StackFrame(const StackFrame &s)
    : caller(s.caller), kf(s.kf), callPathNode(s.callPathNode),
      allocas(s.allocas), snapshot(s.snapshot), trace(s.trace),
      regressionTrace(s.regressionTrace), lazyTrace(s.lazyTrace),
...
SymbolicObjectRecord::SymbolicObjectRecord(const MemoryObject *mo,
                                           const Array *array)
    : mo(mo), array(array), type(""), name(array->getName()), loc(""),
      value(""), size(mo->size), offset(0), address(mo->address) {}

SymbolicObjectRecord::SymbolicObjectRecord(const Array *array, uint64_t size,
                                           uint64_t address, uint64_t base)
    : mo(nullptr), array(array), type(""), name(array->getName()), loc(""),
      value(""), size(size), offset(address - base), address(address) {}

void SymbolicObjectRecord::setHeap() { type = "heap"; }

void SymbolicObjectRecord::setSymPtr(uint64_t concAddr) {
  type = "sym_ptr";
  address = concAddr;
}

void SymbolicObjectRecord::setArg(uint32_t index) {
  type = "arg";
  this->index = index;
}

void SymbolicObjectRecord::setLazy(std::string loc) {
  type = "lazy";
  loc = loc;
}

/***/
int ExecutionState::nextID = 0;
int ExecutionState::nextCrashID = 1;

ExecutionState::ExecutionState(KFunction *kf)
    : metaData(), pc(kf->instructions), prevPC(pc), weight(1), depth(0),
      passedTarget(false), passedCrashLoc(false), taintMarker(false),
      recursiveDepth(0), instsSinceCovNew(0), coveredNew(false),
      forkDisabled(false), ptreeNode(0), steppedInstructions(0),
      functionStateInfo(new FunctionStateInfo()) {
  pushFrame(0, kf);
  setID();
  SPDLOG_DEBUG("[state {}] Creating new state", this->getID());
  setCrashID(0);
}

ExecutionState::ExecutionState(const std::vector<ref<Expr>> &assumptions)
    : metaData(), constraints(assumptions), ptreeNode(0), passedTarget(false),
      passedCrashLoc(false), taintMarker(false), recursiveDepth(0),
      functionStateInfo(new FunctionStateInfo()) {}

ExecutionState::~ExecutionState() {
  for (unsigned int i = 0; i < symbolics.size(); i++) {
    const MemoryObject *mo = symbolics[i].getMemoryObject();
    if (!mo)
      continue;
...
ExecutionState::ExecutionState(const ExecutionState &state)
    : fnAliases(state.fnAliases), pc(state.pc), prevPC(state.prevPC),
      metaData(state.metaData), passedTarget(state.passedTarget),
      passedCrashLoc(state.passedCrashLoc), taintMarker(false),
      recursiveDepth(state.recursiveDepth), stack(state.stack),
      ...
      symbolicFields(state.symbolicFields),
      trackedSymbolicObjects(state.trackedSymbolicObjects),

      queryCost(state.queryCost), cost(time::Span()), weight(state.weight),
      depth(state.depth),
      ...
      arrayNames(state.arrayNames), memTraces(state.memTraces),
      openMergeStack(state.openMergeStack),
      steppedInstructions(state.steppedInstructions),
      functionStateInfo(state.functionStateInfo->copy()) {
  setID();
  SPDLOG_DEBUG("[state {}] Copying [state {}]", this->getID(), state.getID());
  for (unsigned int i = 0; i < symbolics.size(); i++)
    symbolics[i].updateRefCount();
...
ExecutionState *ExecutionState::branch() {
  ...
  SPDLOG_DEBUG("[state {}] -> [state {}] Branching state", this->getID(),
               falseState->getID());
  klee_log_data(fmt::format("[fork] [state {}] -> [state {}]", this->getID(),
                            falseState->getID()));
  weight *= .5;
...
void ExecutionState::addSymbolic(const MemoryObject *mo, const Array *array) {
  if (mo)
    mo->refCount++;
  symbolics.push_back(SymbolicObjectRecord(mo, array));
}

void ExecutionState::addSymbolic(const Array *array, uint64_t size,
                                 uint64_t address, uint64_t base) {
  symbolics.push_back(SymbolicObjectRecord(array, size, address, base));
}
  ...
  if (metaData.isTypeBase()) {
    ...
    if ((pc->inst->getFunction()->getName().str() == "__cpr_choice") || ((!filename.endswith("uni_klee_runtime_new.c")) &&
      ...
      SPDLOG_DEBUG("[state {}] Skipping constraint: {} in {} - taint {}",
                   this->getID(), e->str(), pc->getSourceLocation(),
                   taintMarker);
    }
  }
}

void ExecutionState::addCexPreference(ref<Expr> cond) {
  cexPreferences.push_back(cond);
}

void ExecutionState::addIntBoundConstraint(ref<Expr> e) {
  if (false && e->getWidth() == Expr::Int64) {
    ref<Expr> lowerBound = ConstantExpr::create(0, Expr::Int64);
    ref<Expr> upperBound = ConstantExpr::create(0xFFFFFFFF, Expr::Int64);
    ref<Expr> lowerBoundConstraint = UleExpr::create(lowerBound, e);
    ref<Expr> upperBoundConstraint = UleExpr::create(e, upperBound);
    addConstraint(lowerBoundConstraint);
    addConstraint(upperBoundConstraint);
    SPDLOG_DEBUG("[state {}] Adding int bound constraint: {} -> {}, {}",
                 this->getID(), e->str(), lowerBoundConstraint->str(),
                 upperBoundConstraint->str());
    ref<Expr> readExpr32 = ExtractExpr::create(e, 0, Expr::Int32);
    ref<Expr> readExpr64 = ZExtExpr::create(readExpr32, Expr::Int64);
    ref<Expr> eq = EqExpr::create(e, readExpr64);
    addConstraint(eq);
    SPDLOG_DEBUG("[state {}] Adding int bound constraint: {} -> {}",
                 this->getID(), e->str(), eq->str());
  }
}
...
void ExecutionState::dumpStackTrace(std::vector<std::string> &out,
  ...
  for (ExecutionState::stack_ty::const_reverse_iterator it = stack.rbegin(),
    ...
    if (!target->info->file.empty())
      out.push_back(target->getSourceLocation());
    target = sf.caller;
  }
}
...
void ExecutionState::setTaintMarker(bool taint) {
  if (taintMarker && taint) {
    SPDLOG_WARN("[state {}] Taint marker already set to true", this->getID());
  } else if (taintMarker || taint) {
    SPDLOG_DEBUG("[state {}] Setting taint marker from {} to {}", this->getID(),
                 taintMarker, taint);
  }
  taintMarker = taint;
}