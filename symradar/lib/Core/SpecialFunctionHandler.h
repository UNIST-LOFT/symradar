...
public:
  ...
  void trackMemory(ExecutionState &state, llvm::Type *type, ref<Expr> address,
                   ref<Expr> value);
...
#define HANDLER(name)                                                          \
  ...
  HANDLER(handleUniMakeSymbolic);
  HANDLER(handleExtractFixMakeSymbolic);
  HANDLER(handleRestoreConcrete);
  HANDLER(handleUniKleeAddPatch);
  HANDLER(handleUniKleePocChoice);
  HANDLER(handleUniKleePocRecordValue);
  HANDLER(handleMalloc);
  HANDLER(handleMarkGlobal);
  HANDLER(handleOpenMerge);
  HANDLER(handleSlicing);
