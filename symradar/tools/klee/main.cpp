...
void KleeHandler::setLogger() {
  ...
  spdlog::sinks_init_list sinks = {console, file_debug, file_info, file_warn};
...
int main(int argc, char **argv, char **envp) {
  ...
  llvm::Module *mainModule = loadedModules[0].get();
// Push the module as the first entry
// loadedModules.emplace_back(std::move(M));
#ifdef ENABLE_DG

  std::unique_ptr<llvm::Module> success = slice(loadedModules[0].get());
// if (success) {
